...
/* i assume this how system V func args <corespond> did i get the spelling ? */
...
struct Registerm {
	union {
		char *rdi, *rsi, *rdx, *rcx, *r8, *r9;
	} __64__;
	union {
		char *edi, *esi, *edx, *ecx, *r8d, *r9d;
	} __32__;
};
	...
	switch (node->kind) {
		case ND_VAR:
			...
			if (opt_fpic) {
				...
				println("  mov %s@GOTPCREL(%%rip), %%rax",
					node->var->name);
				return;
			}
			...
			if (node->var->is_tls) {
				...
				println("  add $%s@tpoff, %%rax",
					node->var->name);
				return;
			}

			// Here, we generate an absolute address of a function
			// or a global variable. Even though they exist at a
			// certain address at runtime, their addresses are not
			// known at link-time for the following two reasons.
			//
			//  - Address randomization: Executables are loaded to
			//  memory as a
			//    whole but it is not known what address they are
			//    loaded to. Therefore, at link-time, relative
			//    address in the same exectuable (i.e. the distance
			//    between two functions in the same executable) is
			//    known, but the absolute address is not known.
			//
			//  - Dynamic linking: Dynamic shared objects (DSOs) or
			//  .so files
			//    are loaded to memory alongside an executable at
			//    runtime and linked by the runtime loader in
			//    memory. We know nothing about addresses of global
			//    stuff that may be defined by DSOs until the
			//    runtime relocation is complete.
			//
			// In order to deal with the former case, we use
			// RIP-relative addressing, denoted by `(%rip)`. For the
			// latter, we obtain an address of a stuff that may be
			// in a shared object file from the Global Offset Table
			// using `@GOTPCREL(%rip)` notation.
...
static void load(Type *ty) {
	switch (ty->kind) {
		...
		case TY_FUNC:
		case TY_VLA:
			// If it is an array, do not attempt to load a value to
			// the register because in general we can't load an
			// entire array to a register. As a result, the result
			// of an evaluation of an array becomes not the array
			// itself but the address of the array. This is where
			// "array is automatically converted to a pointer to the
			// first element of the array in C" occurs.
			return;
		case TY_FLOAT:
...
static void copy_ret_buffer(Obj *var) {
	...
	if (ty->size > 8) {
		if (has_flonum2(ty)) {
			...
			if (ty->size == 12)
				println("  movss %%xmm%d, %d(%%rbp)", fp,
					var->offset + 8);
			else
				println("  movsd %%xmm%d, %d(%%rbp)", fp,
					var->offset + 8);
		} else {
			char *reg1 = (gp == 0) ? "%al" : "%dl";
			char *reg2 = (gp == 0) ? "%rax" : "%rdx";
			for (int i = 8; i < MIN(16, ty->size); i++) {
				println("  mov %s, %d(%%rbp)", reg1,
					var->offset + i);
...
static void gen_expr(Node *node) {
	...
	switch (node->kind) {
		...
		case ND_ASSIGN:
			...
			if (node->lhs->kind == ND_MEMBER &&
				...
				// If the lhs is a bitfield, we need to read the
				// current value from memory and merge it with a
				// new value.
		...
		case ND_CAST:
			...
			return;
		case ND_MEMZERO:
			// `rep stosb` is equivalent to `memset(%rdi, %al,
			// %rcx)`.
		...
		case ND_FUNCALL: {
			...
			// If the return type is a large struct/union, the
			// caller passes a pointer to a buffer as if it were the
			// first argument.
			if (node->ret_buffer && node->ty->size > 16)
				pop(argreg64[gp++]);
			...
			// It looks like the most significant 48 or 56 bits in
			// RAX may contain garbage if a function return type is
			// short or bool/char, respectively. We clear the upper
			// bits here.
			switch (node->ty->kind) {
				case TY_BOOL:
					println("  movzx %%al, %%eax");
			...
			// If the return type is a small struct, a value is
			// returned using up to two registers.
			if (node->ret_buffer && node->ty->size <= 16) {
	...
	switch (node->lhs->ty->kind) {
		...
		case TY_DOUBLE: {
			...
			char *sz =
			    (node->lhs->ty->kind == TY_FLOAT) ? "ss" : "sd";
	...
	switch (node->kind) {
		...
		case ND_SWITCH:
			...
			for (Node *n = node->case_next; n; n = n->case_next) {
				char *ax = (node->cond->ty->size == 8) ? "%rax"
								       : "%eax";
				char *di = (node->cond->ty->size == 8) ? "%rdi"
								       : "%edi";
				...
				println("  cmp $%ld, %s", n->end - n->begin,
					di);
...
static void assign_lvar_offsets(Obj *prog) {
	for (Obj *fn = prog; fn; fn = fn->next) {
		...
		// Assign offsets to pass-by-register parameters and local
		// variables.
		for (Obj *var = fn->locals; var; var = var->next) {
			if (var->offset)
				continue;

			// AMD64 System V ABI has a special alignment rule for
			// an array of length at least 16 bytes. We need to
			// align such array to at least 16-byte boundaries. See
			// p.14 of
			// https://github.com/hjl-tools/x86-psABI/wiki/x86-64-psABI-draft.pdf.
			int align =
			    (var->ty->kind == TY_ARRAY && var->ty->size >= 16)
				? MAX(16, var->align)
				: var->align;
...
static void emit_data(Obj *prog) {
	for (Obj *var = prog; var; var = var->next) {
		...
		if (opt_fcommon && var->is_tentative) {
			println("  .comm %s, %d, %d", var->name, var->ty->size,
				align);
			continue;
		}
...
static void emit_text(Obj *prog) {
	for (Obj *fn = prog; fn; fn = fn->next) {
		...
		if (fn->va_area) {
			...
			// va_elem
			println("  movl $%d, %d(%%rbp)", gp * 8,
				off);  // gp_offset
			println("  movl $%d, %d(%%rbp)", fp * 8 + 48,
				off + 4);  // fp_offset
			println("  movq %%rbp, %d(%%rbp)",
				off + 8);  // overflow_arg_area
		...
		for (Obj *var = fn->params; var; var = var->next) {
			...
			switch (ty->kind) {
				...
				case TY_UNION:
					assert(ty->size <= 16);
					if (has_flonum(ty, 0, 8, 0))
						store_fp(fp++, var->offset,
							 MIN(8, ty->size));
					else
						store_gp(gp++, var->offset,
							 MIN(8, ty->size));
		...
		// end of the main function is equivalent to returning 0, even
		// though the behavior is undefined for the other functions.
