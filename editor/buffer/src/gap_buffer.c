...
gapbuffer is free software: you can redistribute it and/or modify it under the
terms of the GNU General Public License as published by the Free Software
Foundation, either version 3 of the License, or (at your option) any later
version.

gapbuffer is distributed in the hope that it will be useful, but WITHOUT ANY
WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS FOR A
PARTICULAR PURPOSE. See the GNU General Public License for more details.

You should have received a copy of the GNU General Public License along with
gapbuffer. If not, see <https://www.gnu.org/licenses/>. */
...
static bool expandGap(GapBuffer **buffer, size str_len) {
	...
	return_value_if(str_len > PTRDIFF_MAX / 2, false,
			ERR_ARITHEMATIC_OVERFLOW);
	return_value_if(
	    orig_buffer->buffer_size < 0 || orig_buffer->gap_len < 0, false,
	    ERR_ARITHEMATIC_OVERFLOW);
	return_value_if(orig_buffer->buffer_size - orig_buffer->gap_len >
			    PTRDIFF_MAX / (2 * str_len),
...
bool GapBuffer_delete(GapBuffer *buffer, usize position, size bytes) {
	const size total_bytes =
	    (buffer->buffer_size - buffer->gap_len) - position;
...
bool GapBuffer_replace(GapBuffer **buffer, const s8 string, usize position) {
	return_value_if(
	    GapBuffer_delete(*buffer, position, string.len) == false, false,
	    ERR_INVALID_SIZE);
	return_value_if(GapBuffer_insert(buffer, string, position) == false,
			false, ERR_OUT_OF_MEMORY);
	return true;
}
...
s8 GapBuffer_getBufferData(GapBuffer *buffer) {
	...
	return_value_if(concat_str.data == NULL, concat_str,
			ERR_OBJECT_INITIALIZATION_FAILED);

	return concat_str;
}

static s8 getDataBeforeGap(GapBuffer *buffer) {
	const s8 before_str = { .data = buffer->data,
...
static s8 getDataAfterGap(GapBuffer *buffer) {
	...
	const s8 after_str = { .data = buffer->data + first_byte_after_gap,
			       .len =
				   buffer->buffer_size - first_byte_after_gap };
