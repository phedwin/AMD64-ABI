...
dloader_p api_load(const char *filename) {
	...
	if (ehdr.e_ident[EI_MAG0] != ELFMAG0 ||
	    ...
	    ehdr.e_ident[EI_MAG3] != ELFMAG3 || ehdr.e_version != EV_CURRENT ||
	...
	if (ehdr.e_phnum > sizeof(phdr) / sizeof(phdr[0]) || ehdr.e_phnum < 1)
		fail(filename, "ELF file has unreasonable ", "e_phnum",
		     ehdr.e_phnum);
	...
	if (first_load->p_offset > ehdr.e_phoff ||
		...
		fail(filename,
		     "First load segment of ELF does not contain phdrs!", NULL,
		     0);
	...
	ElfW(Addr) last_end =
	    first_load->p_vaddr + load_bias + first_load->p_memsz;
	...
	for (const ElfW(Phdr) *ph = first_load + 1; ph <= last_load; ++ph) {
		if (ph->p_type == PT_LOAD) {
			...
			ElfW(Addr) start =
			    round_down(ph->p_vaddr + load_bias, pagesize);
	...
	for (i = 0; i < relocs_size / sizeof(ElfW_Reloc); i++) {
		...
		switch (reloc_type) {
			...
			case R_AARCH64_RELATIVE: {
				ElfW(Addr) *addr =
				    (ElfW(Addr) *)(load_bias + reloc->r_offset);
				/*
				 * If addr loactes in read-only PT_LOAD section,
				 * i.e., .text, then we give the memory fragment
				 * WRITE permission during relocating its
				 * address. Reset its access permission after
				 * relocation to avoid some secure issue.
				 */
				if ((intptr_t)addr < ro_end &&
				    (intptr_t)addr >= ro_start) {
					mprotect((void *)round_down(
						     (intptr_t)addr, pagesize),
						 pagesize, PROT_WRITE);
					*addr += load_bias;
					mprotect((void *)round_down(
						     (intptr_t)addr, pagesize),
						 pagesize,
						 prot_from_phdr(ro_load));
				} else
					*addr += load_bias;
				break;
			}
	...
	if (pltgot != 0) {
		...
		o->dt_jmprel =
		    (ElfW_Reloc *)(get_dynamic_entry(dynamic, DT_JMPREL) +
				   load_bias);
	}
