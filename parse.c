...
static void array_initializer2(Token **rest,
			       Token *tok,
			       Initializer *init,
			       int i);
static void struct_initializer2(Token **rest,
				Token *tok,
				Initializer *init,
				Member *mem);
static void initializer2(Token **rest, Token *tok, Initializer *init);
static Initializer *initializer(Token **rest,
				Token *tok,
				Type *ty,
				Type **new_ty);
...
static Type *declspec(Token **rest, Token *tok, VarAttr *attr) {
	...
	while (is_typename(tok)) {
		...
		if (equal(tok, "typedef") || equal(tok, "static") ||
			...
			if (!attr)
				error_tok(tok,
					  "storage class specifier is not "
			...
			if (attr->is_typedef &&
				...
				1)
				error_tok(tok,
					  "typedef may not be used together "
					  "with static,"
					  " extern, inline, __thread or "
					  "_Thread_local");
			tok = tok->next;
			continue;
		}
		...
		if (equal(tok, "struct") || equal(tok, "union") ||
		    equal(tok, "enum") || equal(tok, "typeof") || ty2) {
			if (counter)
				break;
...
static Type *func_params(Token **rest, Token *tok, Type *ty) {
	...
	while (!equal(tok, ")")) {
		...
		if (ty2->kind == TY_ARRAY) {
			// "array of T" is converted to "pointer to T" only in
			// the parameter context. For example, *argv[] is
			// converted to **argv by this.
...
static Node *compute_vla_size(Type *ty, Token *tok) {
	...
	Node *expr =
	    new_binary(ND_ASSIGN, new_var_node(ty->vla_size, tok),
		       new_binary(ND_MUL, ty->vla_len, base_sz, tok), tok);
	return new_binary(ND_COMMA, node, expr, tok);
}

static Node *new_alloca(Node *sz) {
	Node *node = new_unary(ND_FUNCALL,
			       new_var_node(builtin_alloca, sz->tok), sz->tok);
...
// expr)?)*)? ";"
static Node *declaration(Token **rest,
			 Token *tok,
			 Type *basety,
			 VarAttr *attr) {
	Node head = {};
	...
	while (!equal(tok, ";")) {
		...
		if (ty->kind == TY_VLA) {
			if (equal(tok, "="))
				error_tok(tok,
					  "variable-sized object may not be "
					  "initialized");

			// Variable length arrays (VLAs) are translated to
			// alloca() calls. For example, `int x[n+2]` is
			// translated to `tmp = n + 2, x = alloca(tmp)`.
			Obj *var = new_lvar(get_ident(ty->name), ty);
			Token *tok = ty->name;
			Node *expr = new_binary(
			    ND_ASSIGN, new_vla_ptr(var, tok),
...
static void string_initializer(Token **rest, Token *tok, Initializer *init) {
	if (init->is_flexible)
		*init = *new_initializer(
		    array_of(init->ty->base, tok->ty->array_len), false);
...
static void array_designator(
    Token **rest, Token *tok, Type *ty, int *begin, int *end) {
	*begin = const_expr(&tok, tok->next);
	...
	if (equal(tok, "...")) {
		*end = const_expr(&tok, tok->next);
		if (*end >= ty->array_len)
			error_tok(
			    tok, "array designator index exceeds array bounds");
		if (*end < *begin)
			error_tok(tok,
				  "array designator range [%d, %d] is empty",
				  *begin, *end);
	} else {
		*end = *begin;
	}
...
static Node *init_desg_expr(InitDesg *desg, Token *tok) {
	...
	if (desg->member) {
		Node *node =
		    new_unary(ND_MEMBER, init_desg_expr(desg->next, tok), tok);
...
static Node *create_lvar_init(Initializer *init,
			      Type *ty,
			      InitDesg *desg,
			      Token *tok) {
	if (ty->kind == TY_ARRAY) {
		...
		for (int i = 0; i < ty->array_len; i++) {
			...
			Node *rhs = create_lvar_init(init->children[i],
						     ty->base, &desg2, tok);
	...
	if (ty->kind == TY_STRUCT && !init->expr) {
		...
		for (Member *mem = ty->members; mem; mem = mem->next) {
			...
			Node *rhs = create_lvar_init(init->children[mem->idx],
						     mem->ty, &desg2, tok);
			node = new_binary(ND_COMMA, node, rhs, tok);
		}
		return node;
	...
	if (ty->kind == TY_UNION) {
		...
		InitDesg desg2 = { desg, 0, mem };
		return create_lvar_init(init->children[mem->idx], mem->ty,
					&desg2, tok);
	}
...
static Relocation *write_gvar_data(
	...
	if (ty->kind == TY_ARRAY) {
		int sz = ty->base->size;
		for (int i = 0; i < ty->array_len; i++)
			cur = write_gvar_data(cur, init->children[i], ty->base,
					      buf, offset + sz * i);
		return cur;
	}
	...
	if (ty->kind == TY_STRUCT) {
		for (Member *mem = ty->members; mem; mem = mem->next) {
			if (mem->is_bitfield) {
				...
				uint64_t combined =
				    oldval |
				    ((newval & mask) << mem->bit_offset);
				write_buf(loc, combined, mem->ty->size);
			} else {
				cur = write_gvar_data(
				    cur, init->children[mem->idx], mem->ty, buf,
				    offset + mem->offset);
			}
		}
		return cur;
...
static bool is_typename(Token *tok) {
	...
	if (map.capacity == 0) {
		static char *kw[] = {
			...
			"const",         "volatile",  "auto",
			"register",      "restrict",  "__restrict",
			"__restrict__",  "_Noreturn", "float",
			"double",        "typeof",    "inline",
...
static Node *stmt(Token **rest, Token *tok) {
	...
	if (equal(tok, "goto")) {
		if (equal(tok->next, "*")) {
			// [GNU] `goto *ptr` jumps to the address specified by
			// `ptr`.
	...
	switch (node->kind) {
		...
		case ND_COND:
			if (!is_const_expr(node->cond))
				return false;
			return is_const_expr(eval(node->cond) ? node->then
							      : node->els);
		case ND_COMMA:
...
static Node *to_assign(Node *binary) {
	...
	if (binary->lhs->kind == ND_MEMBER) {
		...
		Node *expr1 =
		    new_binary(ND_ASSIGN, new_var_node(var, tok),
			       new_unary(ND_ADDR, binary->lhs->lhs, tok), tok);

		Node *expr2 = new_unary(
		    ND_MEMBER, new_unary(ND_DEREF, new_var_node(var, tok), tok),
		    tok);
	...
	if (binary->lhs->ty->is_atomic) {
		...
		cur = cur->next = loop;
		cur = cur->next =
		    new_unary(ND_EXPR_STMT, new_var_node(new, tok), tok);
...
static Node *assign(Token **rest, Token *tok) {
	...
	if (equal(tok, "%="))
		return to_assign(
		    new_binary(ND_MOD, node, assign(rest, tok->next), tok));

	if (equal(tok, "&="))
		return to_assign(
		    new_binary(ND_BITAND, node, assign(rest, tok->next), tok));

	if (equal(tok, "|="))
		return to_assign(
		    new_binary(ND_BITOR, node, assign(rest, tok->next), tok));

	if (equal(tok, "^="))
		return to_assign(
		    new_binary(ND_BITXOR, node, assign(rest, tok->next), tok));

	if (equal(tok, "<<="))
		return to_assign(
		    new_binary(ND_SHL, node, assign(rest, tok->next), tok));

	if (equal(tok, ">>="))
		return to_assign(
		    new_binary(ND_SHR, node, assign(rest, tok->next), tok));
...
static Node *conditional(Token **rest, Token *tok) {
	...
	if (equal(tok->next, ":")) {
		...
		Node *lhs =
		    new_binary(ND_ASSIGN, new_var_node(var, tok), cond, tok);
...
static Node *logor(Token **rest, Token *tok) {
	...
	while (equal(tok, "||")) {
		Token *start = tok;
		node =
		    new_binary(ND_LOGOR, node, logand(&tok, tok->next), start);
	}
	*rest = tok;
	return node;
}
...
static Node *logand(Token **rest, Token *tok) {
	...
	while (equal(tok, "&&")) {
		Token *start = tok;
		node =
		    new_binary(ND_LOGAND, node, bitor(&tok, tok->next), start);
	}
	*rest = tok;
	return node;
}
...
static Node *bitor(Token **rest, Token *tok) {
	...
	while (equal(tok, "|")) {
		Token *start = tok;
		node =
		    new_binary(ND_BITOR, node, bitxor(&tok, tok->next), start);
	}
	*rest = tok;
	return node;
}
...
static Node *bitxor(Token **rest, Token *tok) {
	...
	while (equal(tok, "^")) {
		Token *start = tok;
		node =
		    new_binary(ND_BITXOR, node, bitand(&tok, tok->next), start);
	}
	*rest = tok;
	return node;
}
	...
	if (lhs->ty->base->kind == TY_VLA) {
		rhs =
		    new_binary(ND_MUL, rhs,
			       new_var_node(lhs->ty->base->vla_size, tok), tok);
		return new_binary(ND_ADD, lhs, rhs, tok);
	}
...
static Node *new_sub(Node *lhs, Node *rhs, Token *tok) {
	...
	if (lhs->ty->base && is_integer(rhs->ty)) {
		rhs = new_binary(ND_MUL, rhs,
				 new_long(lhs->ty->base->size, tok), tok);
		add_type(rhs);
		Node *node = new_binary(ND_SUB, lhs, rhs, tok);
		node->ty = lhs->ty;
		return node;
...
static Node *mul(Token **rest, Token *tok) {
	...
	for (;;) {
		...
		if (equal(tok, "*")) {
			node = new_binary(ND_MUL, node, cast(&tok, tok->next),
					  start);
			continue;
		}

		if (equal(tok, "/")) {
			node = new_binary(ND_DIV, node, cast(&tok, tok->next),
					  start);
			continue;
		}

		if (equal(tok, "%")) {
			node = new_binary(ND_MOD, node, cast(&tok, tok->next),
					  start);
			continue;
		}
...
static Node *unary(Token **rest, Token *tok) {
	...
	if (equal(tok, "++"))
		return to_assign(
		    new_add(unary(rest, tok->next), new_num(1, tok), tok));

	// Read --i as i-=1
	if (equal(tok, "--"))
		return to_assign(
		    new_sub(unary(rest, tok->next), new_num(1, tok), tok));
...
static void struct_members(Token **rest, Token *tok, Type *ty) {
	...
	if (cur != &head && cur->ty->kind == TY_ARRAY &&
	    cur->ty->array_len < 0) {
...
static Node *new_inc_dec(Node *node, Token *tok, int addend) {
	add_type(node);
	return new_cast(
	    new_add(to_assign(new_add(node, new_num(addend, tok), tok)),
...
static Token *function(Token *tok, Type *basety, VarAttr *attr) {
	...
	if (fn) {
		...
		if (!fn->is_static && attr->is_static)
			error_tok(tok,
				  "static declaration follows a non-static "
				  "declaration");
	...
	push_scope("__func__")->var = new_string_literal(
	    fn->name, array_of(ty_char, strlen(fn->name) + 1));
